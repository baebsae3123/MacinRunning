...
중요하게 4가지만 뽑음 ★★★
🔹 LinearRegression (데이터를 선형관계 연결함으로써 더 좋은 정확도가 나올거라 예상함)
...
🔹 RandomForestRegressor ( 다양한 데이터들이 많으니 여러 트리가 많은 모델이 좋다고 생각함)
...
🔹 XGBRegressor (XGBoost) (데이터들이 정말많고 데이터들이 복잡하게 있으며 패턴을 효과적으로 학습기에 좋은 모델이라 선택함)
...
🔹 SVR (Support Vector Regression) ( 비선형 데이터의 복잡한 패턴을 분석할수있어 다양한 데이터에 유연하게 대응할수있어서 좋은모델이라 선택함) 
